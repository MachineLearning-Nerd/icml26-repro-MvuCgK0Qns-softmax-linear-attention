...
class ReproductionTests(unittest.TestCase):
    ...
    def test_claim_2_exact_falsification_is_fail_closed(self):
        artifact = ROOT / ".openresearch" / "artifacts" / "claim_2"
        raw = artifact / "raw_result.json"
        verifier = artifact / "verify_claim_2.py"
        accepted = subprocess.run(
            [sys.executable, str(verifier), "--raw", str(raw)],
            cwd=ROOT,
            capture_output=True,
        ...
        with tempfile.TemporaryDirectory() as directory:
            tampered = Path(directory) / "tampered.json"
            payload = json.loads(raw.read_text())
            payload["exact_calculation"]["U_lhs_exact"] = "0"
            tampered.write_text(json.dumps(payload))
            rejected = subprocess.run(
                [sys.executable, str(verifier), "--raw", str(tampered)],
                cwd=ROOT,
    ...
    def test_claim_2_independent_checker_and_negative_control(self):
        artifact = ROOT / ".openresearch" / "artifacts" / "claim_2"
        independent = subprocess.run(
            [sys.executable, str(artifact / "independent_check.py")],
            cwd=ROOT,
    ...
    def test_claim_3_symbolic_verification_is_fail_closed(self):
        artifact = ROOT / ".openresearch" / "artifacts" / "claim_3"
        raw = artifact / "raw_result.json"
        verifier = artifact / "verify_claim_3.py"
        accepted = subprocess.run(
            [sys.executable, str(verifier), "--raw", str(raw)],
            cwd=ROOT,
            capture_output=True,
        ...
        with tempfile.TemporaryDirectory() as directory:
            tampered = Path(directory) / "tampered.json"
            payload = json.loads(raw.read_text())
            payload["proof_certificate"]["ratio"] = "V(m-Gamma a)"
            tampered.write_text(json.dumps(payload))
            rejected = subprocess.run(
                [sys.executable, str(verifier), "--raw", str(tampered)],
                cwd=ROOT,
    ...
    def test_claim_3_independent_checker_and_negative_control(self):
        artifact = ROOT / ".openresearch" / "artifacts" / "claim_3"
        independent = subprocess.run(
            [sys.executable, str(artifact / "independent_check.py")],
            cwd=ROOT,
    ...
    def test_claim_5_blocked_verifier_is_fail_closed(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            tampered = Path(directory) / "tampered.json"
            payload = json.loads(raw.read_text())
            payload["verified_subcertificate"]["risk"] = "1"
            tampered.write_text(json.dumps(payload))
            rejected = subprocess.run(
                [sys.executable, str(verifier), "--raw", str(tampered)],
                cwd=ROOT,
    ...
    def test_claim_5_independent_checker_and_negative_control(self):
        artifact = ROOT / ".openresearch" / "artifacts" / "claim_5"
        independent = subprocess.run(
            [sys.executable, str(artifact / "independent_check.py")],
            cwd=ROOT,
