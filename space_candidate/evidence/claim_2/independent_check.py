...
# With L=1, T_hat=V*X: dT_hat/dV=X and dT_hat/dU=0.
# For standard Gaussian population attention T=V*U*Z at U=0,V=1:
# dT/dV=0 and dT/dU=Z.
v_gradient_error = even_standard_gaussian_moment(2)
u_gradient_error = even_standard_gaussian_moment(2)
rhs_log_factor = Fraction(0, 1)  # ln(1) and ln(1)^2 exactly
...
for p in (0, 4, 8):
    for q in (0, 4, 8):
        tilted_moment = moments[p] * moments[q]
        chosen_m_pq = moments[p] * moments[q]
        assert tilted_moment <= chosen_m_pq
